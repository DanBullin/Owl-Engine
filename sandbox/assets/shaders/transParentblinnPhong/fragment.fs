...
{    
	...
	visibility = max(0, pow(2.71828, -0.03*(distanceFromCamera-20)));
    ...
    float transparency = texture(u_diffuseMap[fs_in.TexUnit1], fs_in.TexCoords1).a;
    if(transparency < 0.25) discard;
	
	// check whether result is higher than some threshold, if so, output as bloom threshold color
    float brightness = dot(result, vec3(0.2126, 0.7152, 0.0722));
	//brightness = mix(vec4(0.0, 0.0, 0.0, 1.0), brightness, visibility);
    if(brightness > 1.0)
        BrightColor = vec4(result, transparency) * fs_in.Tint;
    else
        BrightColor = vec4(0.0, 0.0, 0.0, transparency);

	
	FragColor = vec4(result, transparency) * fs_in.Tint;
}