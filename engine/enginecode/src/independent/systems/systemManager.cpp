		...
		if (s_enabled)
		{
			...
			if (s_activeSystems.size() < s_activeSystems.capacity())
			{
				...
				if (!containsSystem(type))
				{
					...
					{
#ifndef NG_DEPLOY
						s_activeSystems.push_back(new Inspector);
						break;
#endif // NG_DEPLOY
					}
					case SystemType::Physics:
					{
						s_activeSystems.push_back(new PhysicsSystem);
						break;
					}
					...
					{
						s_activeSystems.push_back(new AudioManager);
						break;
					}
					case SystemType::ScriptManager:
					{
						s_activeSystems.push_back(new ScriptManager);
						break;
					}
					default:
						break;
