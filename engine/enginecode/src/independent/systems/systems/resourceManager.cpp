...
namespace OwlEngine
{
	...
	*/
	static std::string getConfigAsString(const uint32_t data)
	{
		switch (data)
		{
			...
			case Config::ConfigData::ShowColliders:
				return "[ShowColliders]";
			case Config::ConfigData::ApplyFog:
				return "[ApplyFog]";
			default: return 0;
		}
	}
	...
	void ResourceManager::start()
	{
		if (!s_enabled)
		{
			...
			s_configValues.push_back(configData["showColliders"]);
			s_configValues.push_back(configData["applyFog"]);
		}
	}
	...
	*/
	...
	void ResourceManager::loadTResources()
	{
		...
		ResourceLoader::loadAudio("assets/sounds.json");
