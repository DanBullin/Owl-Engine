		...
		if (s_enabled)
		{
			...
			if (sceneExists(sceneName))
			{
				...
				if (s_activeSceneName != sceneName)
				{
					...
					Window* win = WindowManager::getFocusedWindow();
					if (getActiveScene())
					{
						if (!getActiveScene()->getInspectorEnabled()) getActiveScene()->setViewPort(win->getProperties().getSize());
						else getActiveScene()->setViewPort(getActiveScene()->getViewPort());
					}
