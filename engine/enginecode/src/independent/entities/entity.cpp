...
namespace OwlEngine
{
	...
	*/
	...
	//! setUniqueID()
	/*!
	\param ID a const uint32_t - The unique ID
	*/
	void Entity::setUniqueID(const uint32_t ID)
	{
		m_uniqueID = ID;
	}
	...
	bool Entity::addChildEntity(const std::string& childName, Entity* entity)
	{
		...
		if (!checkChildEntityNameTaken(childName) && childName != "")
		{
			...
			if (childTrans) {
				...
				childTrans->setPosition(childPos - getComponent<Transform>()->getPosition());
			}
			...
			entity->setUniqueID(getParentScene()->getUniqueID());
			getParentScene()->increaseNextEntityID();
			getParentScene()->setEntityListUpdated(true);
			return true;
		}
		else
	...
	*/
	...
	//! setSelected()
	/*!
	\param select a const bool - Should the entity be selected
	*/
	void Entity::setSelected(const bool select)
	{
		m_selected = select;
	}
	...
	//! getSelected()
	/*!
	\return a const bool - Whether the entity is selected
	*/
	...
	//! setSaved()
	/*!
	\param save a const bool - Should the entity be saved
	*/
	void Entity::setSaved(const bool save)
	{
		m_save = save;
	}
	...
	//! getSaved()
	/*!
	\return a const bool - Whether the entity is saved
	*/
	const bool Entity::getSaved() const
	{
		return m_save;
	}
	...
	*/
	nlohmann::ordered_json Entity::getAsJson()
	{
		nlohmann::ordered_json data;

		data["name"] = m_entityName;
		if(m_layer) data["layer"] = m_layer->getLayerName();
		data["display"] = m_display;
		data["save"] = m_save;

		//Loop through each component and add it to the JSON
		for (auto& component : m_components)
		{
			if(component)
				data["components"] += component->getAsJSON();
		}
		...
		for (auto& child : m_childEntities)
		{
			if (child.second)
			{
				if(child.second->getSaved())
					data["childEntities"] += child.second->getAsJson();
			}
		}
