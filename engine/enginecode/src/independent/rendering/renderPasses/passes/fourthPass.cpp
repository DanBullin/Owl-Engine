...
namespace OwlEngine
{
	...
	FourthPass::FourthPass()
	{
		...
		m_defaultFBO = ResourceManager::getResource<FrameBuffer>("defaultFBO");
		m_finalFBO = ResourceManager::getResource<FrameBuffer>("finalFBO");
	...
	*/
	...
	{
		// We need to set this pass's framebuffer based on whether we're viewing the main game or inspector
		if (m_attachedScene->getInspectorEnabled()) 
			m_frameBuffer = m_finalFBO;
		else 
			m_frameBuffer = m_defaultFBO;
