...
namespace OwlEngine
{
	...
	*/
	...
	//! loadModel()
	/*!
	\param modelPath a const std::string& - The file path of the model
	\param meshes a std::vector<Mesh3D>& - The vector of meshes in the model
	*/
	void AssimpLoader::loadModel(const std::string& modelPath, std::vector<Mesh3D>& meshes)
	{
		...
		{
			ENGINE_ERROR("[AssimpLoader::loadModel] Cannot load model {0} into assimp.", modelPath);
			return;
		}
