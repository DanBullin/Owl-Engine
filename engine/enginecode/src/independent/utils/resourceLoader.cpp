...
namespace OwlEngine
{
	...
	*/
	...
	\param filePath a const std::string& - The path to the current file
	*/
	void ResourceLoader::loadAudio(const std::string & filePath)
	{
		nlohmann::json jsonData = ResourceManager::getJSON(filePath);

		ENGINE_INFO("[ResourceLoader::loadAudio] Loading audio");

		// Go through each sound and load it
		for (auto& sound : jsonData["sounds"])
		{
			// Get the sound name
			std::string name = sound["name"].get<std::string>();

			// Load sound
			AudioManager::loadSound(name, sound["soundFilePath"].get<std::string>(), true, sound["looping"].get<bool>(), sound["stream"].get<bool>(), sound["minDistance"], sound["maxDistance"], toType(sound["rollOff"].get<uint32_t>()));
		}

		// Go through each bank and load it
		for (auto& bank : jsonData["banks"])
		{
			// Get the bank name
			std::string name = bank["name"].get<std::string>();

			//Load banks
			AudioManager::loadBank(bank["soundFilePath"].get<std::string>(), bank["flag"]);
		}

		// Go through each event and load it
		for (auto& event : jsonData["events"])
		{
			// Get the event name
			std::string name = event["name"].get<std::string>();

			//Load events
			AudioManager::loadEvent(name, event["eventPath"].get<std::string>());
		}
	}
}